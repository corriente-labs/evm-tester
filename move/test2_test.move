...
module pocvm::test2_tests {
    ...
    public entry fun test_1(admin: signer, core_framework: signer) {
        ...
        let calldata = x"0000000000000000000000000000000900000000000000000000000000000002";
        let caller = 0xc000;
        let to = 0xc001;
        let val = 1000000;
        let output = vm::execute(vm_id, caller, to, val, &calldata, &code);
        assert!(output == x"0000000000000000000000000000000b", 0);
